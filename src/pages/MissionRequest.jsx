...
function MissionRequest() {
  ...
  const [formData, setFormData] = useState({
    ...
    position: '',
    department: '',
    vacationType: VACATION_TYPES.MISSION,
    startDate: '',
    endDate: '',
    days: 1,
    reason: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const vacationData = {
        ...
        missionLocation: formData.missionLocation,
      ...
      setToast({ message: 'تم تقديم طلب المأمورية بنجاح', type: 'success' });
    ...
    <div>
      ...
      <div className="card">
        ...
        <div style={{ 
          ...
          <h3 style={{ margin: '0 0 10px 0', fontSize: '24px' }}>المأمورية</h3>
          <p style={{ margin: 0, fontSize: '18px', opacity: 0.9 }}>
            لا تحتسب من رصيد الإجازات - غير محدودة
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label>جهة المأمورية (اختياري)</label>
            <input
              type="text"
              value={formData.missionLocation}
              onChange={(e) => setFormData(prev => ({ ...prev, missionLocation: e.target.value }))}
              placeholder="مثال: وزارة التخطيط - بغداد"
            />
          </div>

          <div className="form-group">
            <label>الغرض من المأمورية (اختياري)</label>
            <textarea
              value={formData.missionPurpose}
              onChange={(e) => setFormData(prev => ({ ...prev, missionPurpose: e.target.value }))}
              rows="3"
              placeholder="اكتب الغرض من المأمورية..."
            />
          </div>
          ...
          <div className="form-group">
            <label>إرفاق صورة الأمر الإداري (اختياري) - JPG, PNG</label>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              {!formData.attachment ? (
                <>
                  <input
          ...
          <button 
            ...
            disabled={submitting}
          >
            {submitting ? MESSAGES.LOADING.SUBMITTING : '💼 تقديم طلب المأمورية'}
          </button>
        </form>
      </div>
    </div>
